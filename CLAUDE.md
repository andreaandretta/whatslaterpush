...
- ✅ Tweak UI post-Sprint 7.5 (2026-06-02, PR #25): FAB pulse-ring ora gated su `queueEmpty` derivato (nessun msg in `pending`/`sending`/`paused`/`awaiting_*`) invece di `showOnboardingHints && !fabTapped`; `fabTapped` state rimosso. Guida ogni utente con coda vuota, non solo first-run. Gated su `!messagesLoading` per evitare flash al primo paint. Footer brand ridimensionato: wordmark `text-2xl`→`text-xl`, SVG bolla `w-8 h-8`→`w-6 h-6` (24px) — presente ma non dominante.
