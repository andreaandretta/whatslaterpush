...
- [ ] **Task 61 — 🔒 Attivazione Google Calendar sync (residuo, con Andrea)**: codice in prod (`dc22d50`) e migration APPLICATA (22 ago); CALENDAR_TOKEN_SECRET + NEXT_PUBLIC_APP_URL già su Vercel. Restano: (a) credenziali OAuth su Google Cloud Console (~10 min, SOLO Andrea — runbook `docs/RUNBOOK-calendar-sync-attivazione.md` §a); (b) env `GOOGLE_CALENDAR_CLIENT_ID`/`SECRET` + `CALENDAR_SYNC_ENABLED=true` + redeploy; (c) pg_cron ogni 15 min (runbook §d); (d) smoke test §e con evento di prova.
