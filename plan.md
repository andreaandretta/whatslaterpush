...
- [x] **Task 61 — Attivazione Google Calendar sync**: FATTA 23 ago (credenziali GCP whatslater-506322 + client WhatsLater Web creati via Chrome, env+flag su Vercel, redeploy Ready, pg_cron */15). RESIDUO: smoke test §e (collega dalla dashboard + evento di prova) — rimandato per incidente pairing …2226, farlo a numero ri-accoppiato. ⚠️ Rotazione CRON_SECRET (Task 44): aggiornare entrambi i job pg_cron.
- [ ] **Task 62 — Landing: pattern WA Reminders**: (1) frase-fiducia "il tuo cliente riceve il messaggio dal TUO numero e continui la conversazione dal tuo WhatsApp" nella sezione how-it-works; (2) badge/riga "Funziona con Google Calendar e Calendly" (post-attivazione Task 61); (3) testimonianze con ruolo+città agganciate ai singoli step del flusso (raccogliere dai beta user); riferimento design: wareminders.com (audit 22 ago in decisions.md).
- [ ] **Task 64 — Anti-scollegamento (la stabilità del collegamento È il prodotto)**: ✅ fatto il confirm educativo su "Disconnetti" (23 ago: "i messaggi partono comunque, per rientrare serve il supporto"). Restano: (a) **OTP self-chat v1.5** = il fix strutturale del vicolo cieco 409 (priorità ALZATA dopo l'incidente 23 ago: Andrea stesso c'è cascato con un logout volontario); (b) copy "non serve mai scollegarsi" nel messaggio di benvenuto post-pairing; (c) igiene numeri nel runbook Task 59: churn collega/scollega abbassa la fiducia Meta (sopprime la notifica push di collegamento) — mai testare con cicli ravvicinati sui numeri di produzione.
- [ ] **Task 63 — Guida SEO "Promemoria WhatsApp da Calendly/Google Calendar"**: articolo/pagina in italiano che intercetta le ricerche pre-lancio-nativo ("messaggi ricorrenti whatsapp", "promemoria whatsapp automatici da calendly") — concorrenza in italiano quasi nulla; pubblicare PRIMA dell'attivazione del nativo Meta (finestra anti-Sherlocking, decisions.md 22 ago).
