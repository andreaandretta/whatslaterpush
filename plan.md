...
- [ ] **Task 46 — [CRITICO, security] Account hijack durante un flap.** *Perché:* il guard owner-only su `/api/auth/init` morde SOLO se `connection_status==='open'` (`auth/init:182`). Durante un flap (`close`/`connecting` — routine, c'è un watchdog apposta), una POST non autenticata col numero della vittima passa (crea sessione + `forceDeleteInstance` + create). Al re-pair il webhook autentica **TUTTE** le `pending_auth_sessions` per quel phone (`.eq('phone', ownerPhone)`, `webhook:472-475`), non la sessione specifica → un attaccante si fa emettere un cookie valido 90gg. *Fix:* legare il flip pending→authenticated allo **specifico `sessionId`** iniziato dall'owner (o esigere cookie owner-valido anche in stato non-`open`). *Verifica:* test che due sessioni pending per lo stesso phone → solo quella col sessionId corretto viene autenticata.
