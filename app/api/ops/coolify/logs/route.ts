...
// Evolution API runs as a Coolify *service* on the droplet. This is its resource
// uuid (a Coolify resource id, NOT a secret) — used as the default target when
// ?uuid= is omitted, so the control tower can read Evolution's logs bare.
const DEFAULT_EVOLUTION_UUID = 'pkso00o0ccoc8ccgos8ks4cw';
...
export async function GET(req: NextRequest) {
  ...
  const uuid = (url.searchParams.get('uuid') || DEFAULT_EVOLUTION_UUID).trim();
  ...
  try {
    for (const seg of segments) {
      const res = await coolifyGet(`/${seg}/${uuid}/logs?lines=${lines}`, token, controller.signal);
