...
// One-shot admin operation that backfills whatsapp_contacts.profile_pic_url
// for rows still NULL — using bulk findChats/findContacts (one call per
// endpoint per user). Deliberately avoids /chat/fetchProfile (per-contact)
// because that's what saturated Evolution in v1 and forced photos off.
//
// Auth: CRON_SECRET (same pattern as /api/admin/contacts-stats), via either
// `?secret=…` or `Authorization: Bearer …`.
//
// Safety: users processed serially with a 200ms breather between them, so
...
export async function POST(req: NextRequest) {
  ...
  for (const u of userList) {
    ...
    // Pull only the contacts whose photo we still don't have. If none,
    // there's nothing to backfill for this user — skip without spending
    // a single Evolution call.
    ...
    const missingNums = new Set<string>(
      (missing || []).map((r: any) => r.contact_number).filter(Boolean)
    );
    ...
    // Only persist URLs for contacts that are actually missing a photo —
    // never overwrite a fresher URL captured later by the webhook.
  ...
  return NextResponse.json({
    ...
    total_contacts_missing_photo: totalMissing,
