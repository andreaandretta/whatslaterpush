...
export async function POST(req: Request) {
        // BUG3 FIX: Log every incoming request for debugging
  let rawBody = '';
        try {
          ...
          const data = payload?.data;
                  ...
                  console.log('WEBHOOK: event=' + eventType + ' instance=' + (payload?.instance || 'none'));
          ...
          if (!data.key?.fromMe) {
                      console.log('WEBHOOK: Skipped - not fromMe. remoteJid=' + (data.key?.remoteJid || 'none'));
                      return NextResponse.json({ ok:true });
          }
          ...
          if (!user) {
                      ...
                      // BUG3 FIX: Don't notify here as we don't have instance info yet — just log
          ...
          // BUG3 FIX: Look for pending contact from last 30 min
          ...
          if (insErr) {
                      ...
                      // BUG3 FIX: Notify user with specific error details
