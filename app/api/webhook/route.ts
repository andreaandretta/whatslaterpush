...
function extractContent(raw: string, cmdStart: number, cmdEnd: number): string {
      ...
      if (dateM) { t = t.slice(0, dateM.index) + t.slice(dateM.index + dateM[0].length); }
      else { const fraM = /fra\s+\d+\s*(minuto|minuti|ora|ore|giorno|giorni)/i.exec(t);
                if (fraM) { t = t.slice(0, fraM.index) + t.slice(fraM.index + fraM[0].length); }
                else { const domM = /domani(?:\s+alle?\s+\d{1,2}(?::\d{2})?)?/i.exec(t);
                            if (domM) { t = t.slice(0, domM.index) + t.slice(domM.index + domM[0].length); }
                            else { const alleM = /alle?\s+\d{1,2}(?::\d{2})?/i.exec(t);
                                          if (alleM) { t = t.slice(0, alleM.index) + t.slice(alleM.index + alleM[0].length); } } } }
...
async function createUser(phone: string, instanceName: string): Promise<any> {
      ...
      const { data: newUser, error } = await supabase.from('user_instances').insert({
              phone_number: normalized, instance_name: instanceName,
...
export async function POST(req: Request) {
      try {
        ...
        const senderRaw = (data.key?.remoteJid || '').split('@')[0];
              if (!senderRaw) { console.error('WEBHOOK: remoteJid missing'); return NextResponse.json({ ok:true }); }
        ...
        let user = await findUserByPhone(senderRaw);
              ...
              if (!user) { console.error('WEBHOOK: Cannot create user:', senderRaw); return NextResponse.json({ error: 'Cannot create user' }, { status: 500 }); }
        ...
        // ═══ ISSUE #3: COMMANDS - lista, annulla, cancella, help ═══

        // COMMAND: "lista" - show pending messages
        ...
        // COMMAND: "annulla" / "cancella" (LIFO - last pending)
        ...
        // COMMAND: "cancella N" (specific from list)
        ...
        // COMMAND: "annulla Marco" (by recipient name)
        ...
        // ═══ END COMMANDS — continue with scheduling logic ═══
        ...
        console.log('WEBHOOK: Saved for', ownerPhone, 'to', recName);
              await notifyOwner(instanceName, ownerPhone,
