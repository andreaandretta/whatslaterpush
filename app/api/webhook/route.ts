...
export async function POST(req: Request) {
              try {
                ...
                // LOG DETTAGLIATO: struttura completa messaggio per debug
                const msgKeys = Object.keys(data.message || {});
                              const extMsg = data.message?.extendedTextMessage;
                              const ctxInfo = extMsg?.contextInfo;
                              const quotedMsg = ctxInfo?.quotedMessage;
                              console.log("=== WEBHOOK DEBUG ===");
                              console.log("remoteJid:", data.key?.remoteJid);
                              console.log("fromMe:", data.key?.fromMe);
                              console.log("messageType keys:", JSON.stringify(msgKeys));
                              console.log("hasExtendedText:", !!extMsg);
                              console.log("hasContextInfo:", !!ctxInfo);
                              console.log("hasQuotedMessage:", !!quotedMsg);
                              console.log("quotedMessage keys:", JSON.stringify(Object.keys(quotedMsg || {})));
                              console.log("hasContactMessage:", !!quotedMsg?.contactMessage);
                              if (quotedMsg?.contactMessage) {
                                                console.log("contactMessage displayName:", quotedMsg.contactMessage.displayName);
                                                console.log("contactMessage vcard snippet:", (quotedMsg.contactMessage.vcard || '').substring(0, 200));
                              }
                ...
                // ESTRAZIONE DESTINATARIO con log dettagliati
                let recipientNumber: string | null = null;
                              let recipientName: string | null = null;

                // Caso 1: risposta a un messaggio contatto (vCard) in qualsiasi chat
                if (quotedMsg?.contactMessage) {
                                  recipientName = quotedMsg.contactMessage.displayName || 'Contatto';
                                  const vcard = quotedMsg.contactMessage.vcard || '';
                                  // Prova diversi pattern TEL
                                const telMatch =
                                  ...
                                  console.log("vCard found! name:", recipientName, "number:", recipientNumber, "vcard:", vcard.substring(0, 300));
                } else {
                                  console.log("No vCard in quotedMessage, checking other locations...");
                                  // Caso 2: cerca in altri punti della struttura
                                const altContact = data.message?.contactMessage ||
                                                    data.message?.extendedTextMessage?.contextInfo?.quotedMessage?.contactMessage;
                                  if (altContact) {
                                                      recipientName = altContact.displayName || 'Contatto';
                                                      const vcard = altContact.vcard || '';
                                                      const telMatch = vcard.match(/TEL[:;][^:]*:([+\d\s()-]+)/i) || vcard.match(/TEL[:;]+([+\d\s()-]+)/i);
                                                      if (telMatch) recipientNumber = telMatch[1].replace(/[\s()\-]/g, '');
                                                      console.log("Alt vCard found:", recipientName, recipientNumber);
                                  }
                }

                // Fallback: usa remoteJid come destinatario
                if (!recipientNumber) {
                                  ...
                                  console.log("Fallback to remoteJid:", recipientNumber, "(owner:", ownerPhone, ")");
                }
