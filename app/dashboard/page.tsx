...
export default function DashboardPage() {
    ...
    // P1: instanceName dinamico (SchedWhats-{phone} oppure SchedWhats-Primary)
  ...
  const fetchMessages = useCallback(async (phone?: string) => {
        ...
        } catch(e) { /* ignore */ } finally { setMessagesLoading(false); }
  }, [userPhone]);
  ...
  // P2: auto-detect phone dall'istanza Evolution dopo connessione
  const detectPhone = async (instName: string) => {
        setDetectingPhone(true);
        try {
                const res = await fetch('/api/connect', {
                          method: 'POST',
                          headers: { 'Content-Type': 'application/json' },
                          body: JSON.stringify({ action: 'getPhone', instanceName: instName })
                });
                const r = await res.json();
                if (r.success && r.phone) {
                          console.log('Auto-detected phone:', r.phone);
                          savePhone(r.phone);
                          saveInstance(instName);
                          setUserPhone(r.phone);
                          fetchMessages(r.phone);
                }
        } catch(e) {
                console.error('detectPhone err:', e);
        } finally {
  ...
  const handleCancel = async (id) => {
        try {
                await fetch('/api/messages', {
                          method:'PATCH',
                          headers:{'Content-Type':'application/json'},
                          body: JSON.stringify({ id, status:'cancelled', phone: userPhone })
                });
                fetchMessages();
        } catch(e) {}
  };
        ...
        try {
                ...
                if (r.success) {
                          ...
                          if (st==='open'||st==='connected') {
                                      ...
                                      setPairingCode(null);
                                      if(refreshTimer.current) clearInterval(refreshTimer.current);
                                      // Auto-detect phone se non già salvato
  ...
  const handleGetCode = async () => {
        ...
        setError(null);
        try {
                // P1: se abbiamo già un phone salvato, usa istanza dedicata
          const storedPhone = getStoredPhone();
                const res = await fetch('/api/connect', {
                          ...
                          body: JSON.stringify({
                                      action: 'getCode',
                                      phoneNumber: storedPhone || null,
                                      instanceName: storedPhone ? null : instanceName
                          })
                });
                const r = await res.json();
                if (r.success && r.data) {
                          // Aggiorna instanceName dinamico
                  if (r.data.instanceName) {
                              setInstanceName(r.data.instanceName);
                              saveInstance(r.data.instanceName);
                  }
                  ...
                  refreshTimer.current = setInterval(async () => {
                              try {
                                            ...
                                            if (st==='open'||st==='connected') {
                                                            ...
                                                            setPairingCode(null);
                                                            if(refreshTimer.current) clearInterval(refreshTimer.current);
                                                            // P2: auto-detect phone
                                              if (!getStoredPhone()) {
                                                                await detectPhone(curInst);
                                              } else {
                                                                fetchMessages();
                                              }
                                                            return;
                                            ...
                                            // Aggiorna QR
                                const rr = await fetch('/api/connect', {
                                                method:'POST',
                                                headers:{'Content-Type':'application/json'},
  ...
  const C = React.createElement;
    ...
    const getDaysLeft = () => !subscription.trial_ends_at ? 0 :
          Math.max(0, Math.ceil((new Date(subscription.trial_ends_at).getTime()-Date.now())/86400000));

  // P2+P4: se non c'e phone, mostra prompt manuale come fallback
  const renderPhonePrompt = () => {
        if (userPhone) return null;
        if (detectingPhone) return C('div', { className:'bg-blue-50 border border-blue-200 rounded-2xl p-4 mb-6' },
                                           C('p', { className:'text-sm text-blue-700 animate-pulse' }, '🔍 Rilevamento numero in corso...')
                                         );
        return C('div', { className:'bg-yellow-50 border border-yellow-200 rounded-2xl p-4 mb-6' },
                       C('p', { className:'text-sm text-yellow-700 mb-2 font-medium' }, '📱 Numero non rilevato automaticamente. Inseriscilo manualmente:'),
                       C('div', { className:'flex gap-2' },
                                 C('input', {
                                             type:'text',
                                             placeholder:'es. 393401234567',
                                             className:'flex-1 px-3 py-2 border rounded-xl text-sm',
                                             id:'phone-input-manual'
                                 }),
                                 C('button', {
                                             onClick: () => {
  ...
  const renderBanner = () => {
        if (subscription.expired) return C('div', { className:'bg-red-50 border border-red-200 rounded-2xl p-4 mb-6' },
                                                 C('div', { className:'flex items-center justify-between' },
                                                           C('div', null,
                                                                       C('p', { className:'font-semibold text-red-700' }, 'Trial Scaduto'),
                                                                       C('p', { className:'text-sm text-red-600' }, 'Il tuo periodo di prova è terminato. Abbonati per continuare.')),
        ...
        if (subscription.status==='active') return C('div', { className:'bg-green-50 border border-green-200 rounded-2xl p-3 mb-6' },
                                                           C('p', { className:'text-sm text-green-700' }, '✅ Abbonamento attivo'));
        return null;
  };

  const renderMessages = () => {
        if (!userPhone) return C('div', { className:'bg-white rounded-3xl shadow-sm border border-gray-100 p-12 text-center' },
                                       C('p', { className:'text-gray-500' }, status==='connected'
                                                 ? 'Rilevamento numero in corso...'
                                                 : 'Connetti WhatsApp per iniziare.')
                                     );
        if (subscription.expired) return C('div', { className:'bg-white rounded-3xl shadow-sm border border-gray-100 p-12 text-center' },
                                                 C('p', { className:'text-gray-500' }, 'Messaggi bloccati. Abbonati per continuare.')
                                               );
        if (messagesLoading) return C('div', { className:'bg-white rounded-3xl shadow-sm border border-gray-100 p-12 text-center' },
                                            C('p', { className:'text-gray-400 animate-pulse' }, 'Caricamento...')
                                          );
        if (messages.length===0) return C('div', { className:'bg-white rounded-3xl shadow-sm border border-gray-100 p-12 text-center' },
                                                C('p', { className:'text-gray-500' }, status==='connected'
                                                          ? 'Nessun messaggio. Invia una vCard a Note to Self!'
                                                          : 'Connetti WhatsApp per iniziare.')
                                              );
                                             ...
                                             C('div', { className:'flex items-start justify-between gap-4' },
                                                           C('div', { className:'flex-1 min-w-0' },
                                                                           C('div', { className:'flex items-center gap-2 mb-1' },
                                                                                             C('span', { className:'font-medium text-gray-900 truncate' },
                                                                                                                 (msg.recipient_name||msg.recipient_number||'?') +
                                                                                                                 (msg.recipient_name && msg.recipient_number ? ' ('+msg.recipient_number+')' : '')
                                                                                                               ),
                                                                           ...
                                                                           msg.caption && msg.caption !== msg.parsed_message
                                                                             ? C('p', { className:'text-xs text-gray-400 mb-1' }, 'Originale: '+trunc(msg.caption,60))
                                                                             : null,
                                                                           C('p', { className:'text-xs text-gray-400' }, '📅 '+fmt(msg.scheduled_at))
                                                                         ),
                                                           C('div', { className:'flex gap-2 flex-shrink-0' },
                                                                           msg.status==='pending'
                                                                             ? C('button', { onClick:()=>handleCancel(msg.id), className:'px-3 py-1.5 text-xs font-medium rounded-lg border border-orange-200 text-orange-700 hover:bg-orange-50' }, 'Annulla')
                                                                             : null,
               ...
               C('header', { className:'bg-white border-b border-gray-200 sticky top-0 z-10' },
                       C('div', { className:'max-w-5xl mx-auto px-4 h-16 flex items-center justify-between' },
                                 ...
                                 C('div', { className:'flex items-center gap-3' },
                                             ...
                                             userPhone ? C('button', { onClick: handleLogout, className:'px-3 py-1.5 text-xs font-medium rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50' }, 'Logout') : null
                                           )
                               )
                     ),
                                             ...
                                             C('div', { className:'bg-white rounded-3xl shadow-sm border border-gray-100 p-8' },
                                                           ...
                                                           status==='connected'
                                                             ? C('div', { className:'text-center py-8' },
                                                                                   ...
                                                                                   C('h3', { className:'text-lg font-semibold text-green-700 mb-2' }, 'Connesso!'),
                                                                                   userPhone
                                                                                     ? C('p', { className:'text-sm text-gray-500 mb-4' }, '📱 '+userPhone)
                                                                                     : C('p', { className:'text-sm text-gray-400 mb-4 animate-pulse' }, 'Rilevamento numero...'),
                                                             ...
                                                             : C('div', { className:'space-y-5' },
                                                                                   qrCode
                                                                                     ? C('div', { className:'border-2 border-green-100 rounded-2xl p-6 text-center' },
                                                                                     ...
                                                                                     : null,
                                                                                   pairingCode
                                                                                     ? C('div', { className:'bg-green-50 border border-green-200 rounded-2xl p-6 text-center' },
                                                                                   ...
                                                                                   C('button', {
                                                                                                         ...
                                                                                                         disabled: isLoading,
                                                                                                         className:'w-full h-14 text-lg font-semibold rounded-2xl bg-green-600 text-white hover:bg-green-700 disabled:opacity-50'
