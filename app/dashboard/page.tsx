...
export default function DashboardPage() {
  ...
  // ═══ ISSUE #1 FIX: detectPhone with retry backoff ═══
  const detectPhone = async (instName: string, attempt = 1) => {
          ...
          try {
            ...
            // Owner not yet propagated - retry with backoff
            if (r.retry && attempt < 6) {
                        const delay = attempt * 3000; // 3s, 6s, 9s, 12s, 15s
                      console.log(`getPhone: retry in ${delay}ms (attempt ${attempt})`);
                        setTimeout(() => detectPhone(instName, attempt + 1), delay);
                        return;
            }

            // Max retries: show manual input fallback
            console.warn('getPhone: max retries, manual input needed');
                    setDetectingPhone(false);
          } catch(e) {
  ...
  const handleCancel = async (id) => {
          try {
                    await fetch('/api/messages', { method:'PATCH', headers:{'Content-Type':'application/json'},
  ...
  const handleGetCode = async () => {
          ...
          } catch(e) { setError('Connection failed.'); } finally { setIsLoading(false); }
  };

  const handleDisconnect = async () => {
          try { await fetch('/api/connect', { method:'POST', headers:{'Content-Type':'application/json'},
                                                   body: JSON.stringify({ action:'disconnect', instanceName }) }); } catch(e) {}
  ...
  const renderPhonePrompt = () => {
          ...
          return C('div', { className:'bg-yellow-50 border border-yellow-200 rounded-2xl p-4 mb-6' },
                         ...
                         C('div', { className:'flex gap-2' },
                                   ...
                                   C('button', { onClick: () => { const v = document.getElementById('phone-input-manual')?.value?.trim(); if (v) { savePhone(v); setUserPhone(v); fetchMessages(v); } }, className:'px-4 py-2 bg-green-600 text-white rounded-xl text-sm font-medium' }, 'Carica')
                                 )
                       );
  };
  ...
  const renderMessages = () => {
          if (!userPhone) return C('div', { className:'bg-white rounded-3xl shadow-sm border border-gray-100 p-12 text-center' },
                                         C('p', { className:'text-gray-500' }, status==='connected' ? 'Rilevamento numero in corso...' : 'Connetti WhatsApp per iniziare.'));
          ...
          if (messages.length===0) return C('div', { className:'bg-white rounded-3xl shadow-sm border border-gray-100 p-12 text-center' },
                                                  C('p', { className:'text-gray-500' }, status==='connected' ? 'Nessun messaggio. Invia una vCard a Note to Self!' : 'Connetti WhatsApp per iniziare.'));
                                 ...
                                 C('div', { className:'lg:col-span-1' },
                                             C('div', { className:'bg-white rounded-3xl shadow-sm border border-gray-100 p-8' },
                                                           ...
                                                           status==='connected'
                                                             ? C('div', { className:'text-center py-8' },
                                                                                   ...
                                                                                   userPhone ? C('p', { className:'text-sm text-gray-500 mb-4' }, '📱 '+userPhone)
                                                             ...
                                                             : C('div', { className:'space-y-5' },
                                                                                   ...
                                                                                   C('button', { onClick: handleGetCode, disabled: isLoading,
                                             ...
                                             C('div', { className:'mt-6 bg-white rounded-3xl shadow-sm border border-gray-100 p-6' },
                                                           ...
                                                           C('ol', { className:'text-sm text-gray-500 space-y-1 list-decimal list-inside' },
                                                                           ...
                                                                           C('li', null, 'Scrivi "lista" per vedere i pending, "annulla" per cancellare')
                                                                         )
                                                         )
                                           ),
